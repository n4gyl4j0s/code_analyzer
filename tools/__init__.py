...
def get_all_tools() -> Tuple[List[Tool], str]:
  ...
  KRITIKUS: A 'file_path' argumentumnak a projekt gyökeréhez képest MEGADOTT TELJES RELATÍV ÚTVONALNAK kell lennie, ahogyan az a fájlrendszerben, az `rg -l` parancs kimenetében, vagy az AST adatfájlban (általában `ast_input.jsonl`) szerepel. Csak a fájlnév megadása (pl. "MyController.java") NEM ELEGENDŐ, ha az AST adatok teljes útvonallal vannak indexelve!
  Input formátum: JSON {'file_path': 'TELJES/RELATIV/UTVONAL/MyController.java', 'query_type': '...', ...}.
  Az AST adatok rendkívül részletesek lehetnek (osztályok, metódusok, annotációk, végpont-részletek: útvonal, HTTP metódus, stb.). PRIORITÁS: Strukturális vagy szemantikus kódelemzésre, főleg kontrollerek/végpontok azonosítására használd, ha az AST adatok rendelkezésre állnak.
  Ha nem vagy biztos a pontos útvonalban, először használd az `execute_shell_command` eszközt `find` vagy `rg -l` paranccsal a fájl teljes relatív útvonalának felderítésére. Például: `rg -l "MyController" --glob "*Controller.java"` (ez a parancs teljes elérési utakat ad vissza).
  Támogatott 'query_type'-ok (a 'file_path' mindenhol kötelező a teljes relatív útvonallal):
    1. 'get_method_parameters': Metódus paramétereinek lekérdezése. További kulcsok: 'method_name', (opcionális: 'class_name'). Példa: {"query_type": "get_method_parameters", "file_path": "src/main/java/com/example/MyController.java", "class_name": "MyController", "method_name": "getUserById"}
    2. 'get_element_annotations': Osztály vagy metódus annotációinak lekérdezése. További kulcsok: 'element_name', (opcionális: 'element_type': "class"|"method"). Példa: {"query_type": "get_element_annotations", "file_path": "src/main/java/com/example/MyController.java", "element_name": "MyController", "element_type": "class"}
    3. 'get_class_fields': Osztály mezőinek (fields) lekérdezése. További kulcsok: 'class_name'. Példa: {"query_type": "get_class_fields", "file_path": "src/main/java/com/example/MyService.java", "class_name": "MyService"}
    4. 'find_method_calls': Metódushívások keresése egy metóduson belül. További kulcsok: 'method_name', (opcionális: 'class_name', 'callee_name_filter'). Példa: {"query_type": "find_method_calls", "file_path": "src/main/java/com/example/MyController.java", "class_name": "MyController", "method_name": "processRequest"}
    5. 'get_file_structure_summary': A fájl teljes AST struktúrájának (JSON) lekérdezése. Óvatosan használd, nagy kimenetet eredményezhet. Specifikusabb query_type-ok általában jobbak. Példa: {"query_type": "get_file_structure_summary", "file_path": "src/main/java/com/example/MyUtil.java"}
    6. 'get_controllers_in_file': Az adott fájlban található összes 'controller' szerepkörű osztály listázása (név, annotációk, stb.). Példa: {"query_type": "get_controllers_in_file", "file_path": "src/main/java/com/example/api/UserController.java"}
    7. 'get_endpoints_from_file': Az adott fájlban található összes HTTP végpont részletes listázása (osztály, metódus, teljes útvonal, HTTP metódus, paraméterek). EZ A PREFERÁLT ESZKÖZ VÉGPONTOK FELDERÍTÉSÉRE. Példa: {"query_type": "get_endpoints_from_file", "file_path": "src/main/java/com/example/api/OrderController.java"}
  A válasz JSON formátumú lista vagy dictionary, hiba esetén hibaüzenet."""
        ast_tool = Tool(
            name="query_ast_data",
